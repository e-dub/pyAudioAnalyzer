...
class SoundAnalyzer:
    ...
    def PlotFFT(self, fMax=[]):
        ...
        plt.figure()
        plt.plot(f[1:int(self.nSamples/2)], XVal[1:int(self.nSamples/2)],
    ...
    def PlotPower(self, fMax=[]):
        ...
        plt.plot(self.f[0:int(self.nSamples/2)], 10*np.log10(dB), linewidth=self.LineWidth)
    ...
    def PlotCepstrum(self, tMax=[]):
        ...
        plt.plot(self.t[self.t<=tMax]*1000, self.cept[self.t<=tMax], linewidth=self.LineWidth)
    ...
    def PlotFFTpyFFTW(self, nthreads=2, fMax=[]):
        ...
        import pyfftw
        self.X = pyfftw.builders.fft(self.x, auto_align_input=True,
                                     auto_contiguous=True,
                                     planner_effort='FFTW_ESTIMATE',
                                     threads=nthreads, overwrite_input=True)()
        XVal = np.abs(self.X)
        f = np.linspace(0.0, self.fs, self.nSamples)
        plt.figure()
        plt.plot(f[1:int(self.nSamples/2)], XVal[1:int(self.nSamples/2)],
...
if __name__ == "__main__":
    # Set file to be read
    InputFileList = ["GuitarVG/A-Saite.wav", "Music/57007r.wav",
                     ...
                     "Randoms/Bobbypfeife.wav", "Randoms/Bobbypfeife.ogg",
                     "Frequency/Sine_wave_440.ogg", "MusicalNotes/Middle_C.mid",
                     "MusicalNotes/A440.mid", "MusicalNotes/A440_violin.mid",
                     "WineGlass.wav",
